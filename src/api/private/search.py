...
from db.models import ClassGroup, Club, Student
...
async def global_search(
    ...
    for term in search_terms:
        ...
        term_conditions.append(
            or_(
                func.unaccent(Student.first_name).ilike(func.unaccent(f"%{t}%")),
                func.unaccent(Student.last_name).ilike(func.unaccent(f"%{t}%")),
                func.unaccent(Student.trombint_id).ilike(f"%{t}%"),
                func.unaccent(Student.apartment).ilike(f"%{t}%")
            )
        )
    ...
    class_query = select(ClassGroup).where(
        func.unaccent(ClassGroup.name).ilike(func.unaccent(f"%{escape_like(query_clean)}%"))
    ).limit(10)
        ...
        "apartments": [
            {
                ...
                any(term and term in s.apartment.lower() for term in search_terms)
            )
        ][:5]
    }
